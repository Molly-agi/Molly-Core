...
All tools are registered in `src/ai/agency/tool-handlers/index.ts`. The `MOLLY_DISABLE_TOOLS` env var can strip tools at load time (mirrors Claude Code's DISABLE\_\*\_COMMAND pattern).
