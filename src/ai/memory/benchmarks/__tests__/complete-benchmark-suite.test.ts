...
describe('Complete MODEL_95_NESTED Benchmark Suite with Graphs', () => {
  it('should run all benchmarks and generate comparison report with charts', async () => {
    ...
    console.log('PHASE 1: MULTI-DATA BENCHMARK (synthetic — fat AI, VR, generic)');
    console.log('═'.repeat(80) + '\n');
    ...
    console.log('\n═'.repeat(80));
    console.log('PHASE 3: REAL DATA BENCHMARK (Molly\'s memories + MMLU + docs + logs)');
    console.log('═'.repeat(80) + '\n');
    const realDataResult = await runRealDataBenchmark();

    // Prepare chart data
    const multiDataChartData: ChartData[] = multiDataResult.results.map(r => ({
      testName: 'Multi-Data',
      dataType: r.dataType,
    ...
    // Save report
    const reportPath = path.join(process.cwd(), 'BENCHMARK_REPORT_MODEL_95.html');
    ...
    const jsonData = {
      ...
      summary: {
        ...
        overallAvgCompression: (
          allResults.reduce((a, r) => a + r.compressionRatio, 0) / allResults.length
        ).toFixed(1),
        allTestsPassed: allResults.every(r => r.passed),
        mollyRealMemoryResult: realDataResult.results.find(r => r.dataType === 'MOLLY_REAL_MEMORIES'),
      },
    };
