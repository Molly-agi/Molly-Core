...
describe('MODEL_95_NESTED vs Industry Standards', () => {
  it('should benchmark all data shapes and produce comparison chart', async () => {
    ...
    console.log('Algorithms: gzip (1/6/9), brotli (4/11), deflate (6/9), raw JSON');
    console.log('Datasets:   FLAT_1000  |  NESTED_1000  |  BULK_5000  |  MOLLY_REAL');
    ...
    for (const ds of report.datasets) {
      ...
      const sorted = [...ds.entries].sort((a, b) => b.compressionRatio - a.compressionRatio);
      for (const e of sorted) {
        const recallStr = e.episodicRecall !== null
          ? `recall=${(e.episodicRecall * 100).toFixed(0)}%`
          : 'no-recall-guarantee';
        const marker = e.type === 'model95' ? '  ★' : '   ';
        console.log(`${marker} ${e.algorithm.padEnd(32)} ${e.compressionRatio.toFixed(1).padStart(6)}%  ${recallStr}  (${e.executionMs.toFixed(0)}ms)`);
      }
    }
    ...
    for (const ds of report.datasets) {
      ...
      const outcome = diff >= 0 ? `★ MODEL_95 wins by +${diff.toFixed(1)}%` : `  industry wins by ${Math.abs(diff).toFixed(1)}% (${bestInd.algorithm})`;
      console.log(`  ${ds.datasetName.padEnd(16)}  MODEL_95=${m95.compressionRatio.toFixed(1)}%  best_industry=${bestInd.compressionRatio.toFixed(1)}%  → ${outcome}`);
    }
    console.log('');

    // ── Save JSON ──────────────────────────────────────────────────────────
    const jsonPath = path.join(process.cwd(), 'BENCHMARK_INDUSTRY_COMPARISON.json');
    fs.writeFileSync(jsonPath, JSON.stringify(report, null, 2));
    console.log(`  ✓ Raw data saved: ${jsonPath}`);

    // ── Generate HTML report ───────────────────────────────────────────────
    const html = generateIndustryComparisonReport(report);
    const htmlPath = path.join(process.cwd(), 'BENCHMARK_INDUSTRY_COMPARISON.html');
