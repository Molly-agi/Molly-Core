...
describe("Real Memory Benchmark — Molly's Actual Experiences", () => {
  it("should benchmark all three models on Molly's real memories", async () => {
    ...
    expect(result.results.some((r) => r.model === 'MODEL_95_NESTED')).toBe(
      true
    );
    ...
    expect(nested.compressionRatio).toBeGreaterThanOrEqual(
      flat.compressionRatio
    );
    ...
    console.log(
      `  MODEL_75_VR:   ${vr.compressionRatio.toFixed(1)}% (3 techniques)`
    );
    console.log(
      `  MODEL_85_FLAT: ${flat.compressionRatio.toFixed(1)}% (6 techniques) — +${(flat.compressionRatio - vr.compressionRatio).toFixed(1)}%`
    );
    console.log(
      `  MODEL_95_NESTED: ${nested.compressionRatio.toFixed(1)}% (8 techniques) — +${(nested.compressionRatio - flat.compressionRatio).toFixed(1)}%`
    );
    console.log(
