...
describe('Cognitive Governor', () => {
  it('should admit flows until capacity, suggest preemption, track different kinds separately, enforce limit changes, and reconcile overages across 5 test groups', () => {
    ...
    assert(
      !d.admit && !d.suggestCancel,
      'equal-priority refused, no preempt suggestion'
    );
    ...
    assert(
      !d.admit && !!d.suggestCancel,
      'high-priority gets a victim to cancel'
    );
    assert(
      !d.admit && d.suggestCancel === started[0].id,
      'victim is a lowest-priority active flow'
    );

    // 4. Different kinds have independent caps.
    const tool = gov.shouldStart({
      kind: 'tool',
      type: 'httpRequest',
      priority: 5,
    });
    assert(
      tool.admit,
      'tools have their own cap, unaffected by flow saturation'
    );
    ...
    assert(
      hist.some((h) => h.kind === 'proposal-rejected'),
      'drastic cut rejected and logged'
    );

    console.log('ALL 6 GOVERNOR GROUPS PASSED');
    console.log('snapshot:', JSON.stringify(gov.snapshot(), null, 2));

    expect(gov.snapshot()).toBeDefined();
  });
});
