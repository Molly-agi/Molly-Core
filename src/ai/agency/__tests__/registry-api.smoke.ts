...
const rb = read.body as any;
assert(K.maxConcurrentFlows in rb.snapshot, 'snapshot includes governor params');
assert(typeof rb.governor.limits.flow === 'number', 'governor snapshot present');

// 2. Propose needs no token, returns 202 + queued.
let w = writeRegistry(rt, { action: 'propose', key: K.maxConcurrentFlows, value: 6, actor: 'self-calibration', reason: 'creep' }, undefined, 'secret');
assert(w.status === 202, 'propose queued without token');

// 3. Override with NO server token configured → 503 fail-closed.
w = writeRegistry(rt, { action: 'override', key: K.maxConcurrentFlows, value: 8, actor: 'eric', reason: 'manual' }, 'whatever', undefined);
assert(w.status === 503, 'override disabled when token unconfigured');

// 4. Override with wrong token → 401.
w = writeRegistry(rt, { action: 'override', key: K.maxConcurrentFlows, value: 8, actor: 'eric', reason: 'manual' }, 'wrong', 'secret');
...
w = writeRegistry(rt, { action: 'override', key: K.maxConcurrentFlows, value: 8, actor: 'eric', reason: 'manual bump' }, 'secret', 'secret');
assert(w.status === 200, 'override ok with good token');
assert(rt.registry.get<number>(K.maxConcurrentFlows) === 8, 'override applied');
assert(rt.registry.getHistory(K.maxConcurrentFlows).some((h) => h.kind === 'operator-override'), 'tagged operator-override');

// 6. Override out of bounds → 422 (bounds bind humans too).
w = writeRegistry(rt, { action: 'override', key: K.maxConcurrentFlows, value: 9999, actor: 'eric', reason: 'oops' }, 'secret', 'secret');
...
console.log('ALL 7 API GROUPS PASSED');
