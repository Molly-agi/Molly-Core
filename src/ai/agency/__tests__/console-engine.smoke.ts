...
describe('Console Engine', () => {
  it('should handle ls, get, propose, pending, resolve, override, history, gov, help, and unknown commands across 9 test groups', () => {
    ...
    // 4. pending lists it.
    assert(
      text(run(`pending ${K.maxConcurrentFlows}`)).includes(
        'by console:tester'
      ),
      'pending shows proposer'
    );

    // 5. resolve via governor resolver commits it.
    r = run(`resolve ${K.maxConcurrentFlows}`, {
    ...
    assert(
      r.lines[0].stream === 'err' && text(r).includes('no resolver'),
      'no-resolver refusal'
    );
    ...
    assert(
      run('frobnicate').lines[0].stream === 'err',
