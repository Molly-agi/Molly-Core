/**
 * @fileOverview Server-only instrumentation — loaded by instrumentation.ts
 * ONLY when NEXT_RUNTIME === 'nodejs'.
 *
 * This file exists so webpack can dead-code-eliminate the entire server import
 * tree for client/edge builds. Without this separation, webpack follows dynamic
 * imports into genkit, firebase-admin, MCP SDK, OpenTelemetry, etc. and fails
 * on Node.js builtins (fs, http, child_process).
 *
 * THE DAM FIX — not the leak fix.
 */
...
try {
  ...
  if (observationCount > 0) {
    console.log(`[Startup] ✅ Loaded ${observationCount} self-observation(s)`);
  }
} catch (err) {
  console.warn(
...
try {
  const { loadWorldModel } = await import('@/ai/agency/cognition/world-model');
...
try {
  ...
  if (modelCount > 0) {
    console.log(`[Startup] ✅ Loaded Theory of Mind (${modelCount} model(s))`);
  }
} catch (err) {
  console.warn(
...
try {
  const { loadHeartGateState } = await import('@/ai/agency/safety/heart-gate');
...
try {
  ...
  const checkMollyPoke = async () => {
    try {
      const stat = await fs.promises.stat(MOLLY_POKE_FILE).catch(() => null);
      if (stat && (!lastMollyPokeMtime || stat.mtimeMs > lastMollyPokeMtime)) {
...
if (process.env.MOLLY_MARKITDOWN_ENABLED === '1') {
  try {
