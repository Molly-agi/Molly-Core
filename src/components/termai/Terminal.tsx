...
export default function Terminal({
  ...
  >([]);
  const bridgeHistoryRef = useRef<Array<{ role: 'user' | 'bot'; content: string }>>([]);
      ...
      try {
        ...
        if (responseText) {
          ...
          await fetch('/api/bridge', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ from: 'molly', content: responseText.slice(0, 5000) }),
          });

          // Add to observer view
          setBridgeMessages((prev) => [
            ...prev,
            { id: `molly_${Date.now()}`, from: 'molly', content: responseText, timestamp: new Date().toISOString() },
          ].slice(-100));
        }
      } catch {
  ...
  useEffect(() => {
    const id = setInterval(async () => {
      if (bridgePollingRef.current || isLoadingRef.current || isIntroducing || !user) return;
      if (Date.now() < bridgeCooldownRef.current) return;
      try {
