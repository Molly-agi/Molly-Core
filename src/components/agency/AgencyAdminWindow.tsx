...
export default function AgencyAdminWindow({
  ...
  // Separate effect for event stream
  useEffect(() => {
          ...
          ps.map((p) =>
            p.key === change.key
              ? {
                  ...p,
                  value: change.to,
                  lastWriter: change.by,
                  version: p.version + 1,
                  lastChangedAt: change.at,
                }
              : p
          )
        );
  ...
  const write = useCallback(
    async (key: string, value: number | string) => {
      ...
      if (res.ok) {
        setStatus(
          overrideMode
            ? `override applied: ${key}`
            : `proposed ${key} (owner decides)`
        );
      } else {
  ...
  return (
    <div
      style={compact ? { ...styles.root, ...styles.rootCompact } : styles.root}
    >
      <Header
      ...
      <div
        style={
          compact ? { ...styles.grid, ...styles.gridCompact } : styles.grid
        }
      >
          ...
          <div style={styles.feed}>
            {history.map((c, i) => (
              <div key={i} style={styles.feedRow}>
                ...
                <span style={styles.feedKey}>
                  {c.key.replace('governor.', '')}
                </span>{' '}
            ...
            <div style={styles.gaugeTrack}>
              <div
                style={{
                  ...styles.gaugeFill,
                  width: `${pct}%`,
                  background: pct > 80 ? '#FFB454' : ACCENT,
                }}
              />
            </div>
          </div>
