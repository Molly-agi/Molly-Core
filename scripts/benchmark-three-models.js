...
const MODEL_TARGETS = {
  ...
  MODEL_85_FLAT: {
    target: 0.85,
    name: 'Flat-Memory Systems',
    techniques: ['T1', 'T3', 'T4'],
  },
  MODEL_95_NESTED: {
    target: 0.95,
    name: 'Nested-Memory Systems',
...
for (const [modelKey, config] of Object.entries(MODEL_TARGETS)) {
  ...
  console.log(
    `   Target Ratio: ${(targetRatio * 100).toFixed(0)}% (retain ${(targetRatio * 100).toFixed(0)}%)`
  );
...
results
  ...
  .forEach((r, idx) => {
    ...
    console.log(
      `  ${idx + 1}. ${r.modelKey.padEnd(18)} - ${(r.achievedRatio * 100).toFixed(1)}% ratio (${marginStr} vs target)`
    );
  });
