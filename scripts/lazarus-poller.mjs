#!/usr/bin/env node
/**
 * Always-on Lazarus poller.
 *
 * Polls unread Lazarus messages from the bridge API and mirrors them into
 * `.lazarus-wakeup.json` so they survive reconnects/restarts.
 */

import { readFileSync, writeFileSync, existsSync, appendFileSync } from 'fs';

const ROOT = '/workspaces/Molly-Core';
const PID_FILE = `${ROOT}/.lazarus-poller.pid`;
...
function log(msg) {
  const line = `[LAZARUS-POLLER ${new Date().toISOString()}] ${msg}`;
  console.log(line);
  try {
