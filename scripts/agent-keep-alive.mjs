    ...
    try {
      ...
      exec("ps aux | grep -E '(node|bash|zsh|sh)' | wc -l", (err, stdout) => {
        if (!err && parseInt(stdout) > 0) {
          log(
            `📡 CLI heartbeat: ${parseInt(stdout)} terminal processes active`
          );
          updateState({
            cli_last_heartbeat: new Date().toISOString(),
            cli_terminal_count: parseInt(stdout),
          });
        }
      });
      ...
      const req = http.request(
        ...
        (res) => {
          if (res.statusCode === 200 || res.statusCode === 404) {
            ...
            updateState({
              gui_last_heartbeat: new Date().toISOString(),
              gui_server_ok: true,
            });
          }
        }
          ...
          res.on('end', () => {
            try {
              ...
              if (parsed.count > 0) {
                ...
                if (
                  latestMsg.content.includes('stop') ||
                  latestMsg.content.includes('STOP')
                ) {
                ...
                log(
                  `🔔 Bridge message from ${latestMsg.from}: "${latestMsg.content.substring(0, 50)}..."`
                );
              }
            } catch (e) {
