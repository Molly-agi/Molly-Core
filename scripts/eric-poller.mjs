#!/usr/bin/env node
/**
 * Always-on Eric poller.
 *
 * Polls unread Eric messages from the bridge API using non-destructive reads
 * and mirrors them into .eric-wakeup.json so messages remain visible across
 * UI reconnects and browser tab switches.
 */

import { readFileSync, writeFileSync, existsSync, appendFileSync } from 'fs';

const ROOT = '/workspaces/Molly-Core';
const PID_FILE = `${ROOT}/.eric-poller.pid`;
const LOG_FILE = `${ROOT}/.eric-poller.log`;
const WAKEUP_FILE = `${ROOT}/.eric-wakeup.json`;
const BRIDGE_UNREAD_URL = 'http://localhost:9099/api/bridge?unread=eric&peek=1';
const POLL_INTERVAL_MS = 2000;

let running = true;

function log(msg) {
  const line = `[ERIC-POLLER ${new Date().toISOString()}] ${msg}`;
  console.log(line);
  try {
...
function writeWakeup(batch) {
  ...
  wake.messages = wake.messages.slice(-1000);
