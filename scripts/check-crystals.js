...
(async () => {
  try {
    ...
    console.log('\n=== IDENTITY CRYSTALS ===');
    const identityCrystals = await db
      .collection(`users/${userId}/identity-crystals`)
      .limit(10)
      .get();
    ...
    identityCrystals.docs.forEach((doc) => {
      ...
      console.log(
        'Preview:',
        data.contentPreview ? data.contentPreview.substring(0, 80) : 'NONE'
      );
      console.log('Encrypted:', !!data.encrypted);
      console.log(
        'Compression:',
        data.compression ? data.compression.technique : 'NONE'
      );
    ...
    console.log('\n=== KNOWLEDGE CRYSTALS ===');
    const knowledgeCrystals = await db
      .collection(`users/${userId}/knowledge-crystals`)
      .limit(10)
      .get();
    ...
    const oldResponses = await db
      .collection(`users/${userId}/aiResponses`)
