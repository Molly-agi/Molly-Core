...
function getCodespaceUrl(port) {
  ...
  try {
    const info = execSync(
      'gh codespace view --json name -q .name 2>/dev/null',
      {
        encoding: 'utf8',
        timeout: 5000,
      }
...
function makePortPublic(port) {
  ...
  try {
    ...
    console.log(
      `  ⚠ Could not auto-set port public — set it manually in PORTS tab`
    );
  }
}
...
function buildPage(authUrl, status = 'waiting') {
  const messages = {
    waiting: {
      title: 'Tap to Connect Claude Code',
      btn: '🔑 Connect Claude Code',
      btnUrl: authUrl,
      color: '#7c3aed',
    },
    success: {
      title: '✅ Claude Code Connected!',
      btn: 'You can close this tab',
      btnUrl: '#',
      color: '#16a34a',
    },
    error: {
...
async function main() {
  ...
  if (!baseUrl) {
    ...
    console.log(
      '  You may need to manually set port ' + PORT + ' to public.\n'
    );
  } else {
    ...
    if (pathname === '/callback' || pathname === '/') {
      ...
      if (code && !resolved) {
        ...
        try {
          ...
          console.error(
            '  Try running: claude login  (in a separate terminal)'
          );
          setTimeout(() => {
            server.close();
            process.exit(1);
