#!/usr/bin/env node
/**
 * Always-on Atlas poller.
 *
 * Polls unread Atlas messages from the bridge API and mirrors them into
 * .atlas-wakeup.json so they survive reconnects/restarts.
 */
...
function log(msg) {
  const line = `[ATLAS-POLLER ${new Date().toISOString()}] ${msg}`;
  console.log(line);
  try {
