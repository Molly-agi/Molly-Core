...
**Status as of 2026-06-23:** 4 of 21 done. Skill registry landed via PR #212 (764 skills + 31 agents Molly-owned). Crystallizer feed wired via the neural-engram tail hook for engram writes — every server-side `remember()` call now enqueues a pending moment and pokes AutoDream. Item 1 remains partially open for any experience streams that do not write engrams (bridge POST, tool execution, conversation turns without engram side-effects). Items 3 and 4 closed today — recall was already wired through `base-composer.ts`, now locked by contract test; crystal injection wired and locked end-to-end. The other 17 items remain.
...
4. ✅ **DONE — Crystals wired into prompt assembly (2026-06-23).** Chain: `composeSystemPrompt()` → `buildDynamicSections()` → `volatileSection('crystals', ...)` → `buildCrystalsInjection(userId)` → `buildConversationCrystalContext()` → `loadIdentityCrystalsForConversation()`. Identity crystals are loaded for the active user (when `crystalUserId` is passed) and injected as a sanitized `<crystals>` block alongside `<recalled-memory>`. Returns null silently when no userId, no `ENGRAM_SECRET`, no crystals loaded, or load failure — never breaks prompt assembly. Mirrors the security framing of recall injection: same `sanitizeRecallText` strips control chars + escapes angle brackets so a crystal cannot close its fence early. Locked by `src/ai/prompts/__tests__/crystals-prompt-injection.contract.test.ts` (3 tests: surface, omit-when-no-userId, omit-when-no-crystals). Follow-up (separate slot): pass `crystalUserId` from `conversational-chat.ts` like `recallQuery` so this fires in production, not just in tests.
